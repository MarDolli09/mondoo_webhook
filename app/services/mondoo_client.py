...
class MondooGraphQLClient:
    ...
    async def fetch_asset_name(self, space_id: str, asset_id: str, scope_mrn: str) -> Optional[str]:
        ...
        endpoint = (
            "https://eu.api.mondoo.com/query"
            if space_id.startswith("eu-")
            else "https://api.mondoo.com/query"
        )

        payload = {
            "query": GET_ASSET_QUERY,
            "variables": {"mrn": scope_mrn}
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if name:
                    logger.info(f"Asset-Name via GraphQL geladen: {name}")
                    return name
            else:
                logger.error(f"GraphQL API HTTP-Fehler {response.status_code} für Asset {asset_id}: {response.text}")
    ...
    async def fetch_cvss_details(
        ...
        try:
            logger.info(
                f"Die ersten {min(log_interval, max_pages)} Seiten werden nach CVE '{target_cve}' durchsucht"
            )
            ...
            while has_next_page:
                ...
                if response.status_code != 200:
                    logger.error(f"GraphQL API HTTP-Fehler {response.status_code} für Scope {scope_mrn}: {response.text}")
                    break
                ...
                if "message" in findings_union:
                    logger.error(f"GraphQL API Union-Fehler auf Seite {page_count}: {findings_union.get('message')}")
                    break

                edges = findings_union.get("edges") or []
                ...
                if has_next_page and page_count % log_interval == 0 and page_count < max_pages:
                    ...
                    logger.info(
                        f"CVE '{target_cve}' wurde auf den Seiten {interval_start}-{page_count} "
                        f"nicht gefunden. Starte nächsten Intervall."
                    )
